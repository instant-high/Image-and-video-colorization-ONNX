...
class COLORIZER:
    ...
    def colorize(self, img_bgr, r_factor):
        ...
        img_bgr_rs = cv2.resize(img_bgr_orig,(r_factor,r_factor))
        ...
        img_l_rs = np.expand_dims(img_l_rs, 0).astype(np.float16)
        img_l_rs = np.expand_dims(img_l_rs, 0).astype(np.float16)
        
        # inference:
        out_ab = self.session.run(['output'], input_feed={"input": img_l_rs})[0]  # (1,2,256,256)
        
        # postprocess:
        HW_orig = img_l_orig.shape[:]  # H_orig x W_orig
        HW = out_ab.shape[2:]  # 1 x 2 x H x W
        out_ab = out_ab[0].transpose(1, 2, 0)  # HXWX2
        out_ab = out_ab.astype(np.float32)
